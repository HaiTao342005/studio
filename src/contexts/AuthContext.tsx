...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function setupAuth() {
      ...
      let storedUserFromLocalStorage: User | null = null;
      try {
        const storedCurrentUserJson = localStorage.getItem(CURRENT_USER_STORAGE_KEY);
        if (storedCurrentUserJson) {
          storedUserFromLocalStorage = JSON.parse(storedCurrentUserJson);
        }
      } catch (error) {
      ...
      const fetchAllOrdersForHistory = async () => {
        ...
        try {
          ...
          allOrdersSnap.forEach(orderDoc => allOrderDocsForHistory.push({ id: orderDoc.id, ...orderDoc.data() } as StoredOrder));
          ...
          if (error instanceof FirestoreError && (error.code === 'unavailable' || (error.message && error.message.includes('client is offline')))) {
              ...
              if(isMounted) toast({ title: "Network Issue", description: "Could not fetch order history for rating calculation. Firebase is offline.", variant: "destructive", duration: 7000});
          } else {
              console.error("Error in fetchAllOrdersForHistory:", error);
              if(isMounted) toast({ title: "Order History Error", description: "Could not fetch order history for rating calculation.", variant: "destructive"});
          }
          allOrderDocsForHistory = [];
          customerSupplierPurchaseCount.clear();
        }
      ...
      try {
        ...
        const unsubscribeUsersSnapshot = onSnapshot(usersQuery, async (querySnapshot) => {
          ...
          try {
            ...
            const enrichedUsersPromises = baseUsers.map(async (u) => {
              ...
              if (shouldSuspend && !userWithRatings.isSuspended) {
                ...
                try {
                  if(db) await updateDoc(doc(db, "users", u.id), { isSuspended: true });
                  console.log(`User ${u.name} (${u.id}) automatically suspended due to low ratings.`);
                  if(isMounted && userRef.current?.id === u.id) toast({ title: "Account Suspended", description: 'Your account has been automatically suspended due to consistently low ratings.', variant: "destructive", duration: 10000});
                  ...
                  if (isMounted && db) { 
                     if (suspendError instanceof FirestoreError && (suspendError.code === 'unavailable' || (suspendError.message && suspendError.message.includes('client is offline')))) {
                       ...
                       console.error(`Failed to update suspension for user ${u.id}:`, suspendError);
                       toast({ title: "Suspension Error", description: `Could not update suspension for ${u.name}.`, variant: "destructive"});
                     }
                  } else if (!db) {
                     console.info(`AuthContext: Suspending user ${u.id} skipped as Firestore (db) became unavailable.`);
                  }
                }
              }
            ...
            if (isMounted) {
              ...
              if (storedUserFromLocalStorage) {
                const latestUserData = enrichedUsers.find(u => u.id === storedUserFromLocalStorage!.id);
                if (latestUserData && JSON.stringify(latestUserData) !== JSON.stringify(storedUserFromLocalStorage)) {
              ...
              if (innerError instanceof FirestoreError && (innerError.code === 'unavailable' || (innerError.message && innerError.message.includes('client is offline')))) {
                ...
                if(isMounted) toast({ title: "Network Issue", description: "Could not update user data. Firebase is offline.", variant: "destructive", duration: 7000});
              } else {
                console.error("Error processing user data snapshot:", innerError);
                if(isMounted) toast({ title: "Data Processing Error", description: "Failed to process user data.", variant: "destructive" });
              }
          ...
          if (error instanceof FirestoreError && (error.code === 'unavailable' || (error.message && error.message.includes('client is offline')))) {
            ...
            if(isMounted) toast({ title: "Network Issue", description: "Could not connect to Firebase for live user updates. Some functions may be limited.", variant: "destructive", duration: 10000});
          } else {
          ...
          try {
            ...
            if (currentAllUsersList.length > 0) {
                ...
                const reEnrichedUsersPromises = baseUsersForAssessmentUpdate.map(async (u) => {
                  ...
                  if (shouldSuspend && !userWithRatings.isSuspended) {
                    ...
                    try {
                        ...
                        if(isMounted && userRef.current?.id === u.id) toast({ title: "Account Suspended", description: 'Your account has been automatically suspended.', variant: "destructive", duration: 10000});
                        else if(isMounted) toast({ title: "User Suspended", description: `User ${u.name} has been automatically suspended.`, variant: "destructive", duration: 10000});
                    } catch (suspendError: any) {
                        if(isMounted && db) {
                           if (suspendError instanceof FirestoreError && (suspendError.code === 'unavailable' || (suspendError.message && suspendError.message.includes('client is offline')))) {
                             console.info(`AuthContext: Suspending user ${u.id} (assessment listener) failed as Firestore client is offline.`, suspendError.message);
                             toast({ title: "Network Issue", description: `Could not update suspension for ${u.name}. Firebase is offline.`, variant: "destructive", duration: 7000});
                           } else {
                             console.error(`Failed to update suspension for ${u.id} (assessment listener):`, suspendError);
                             toast({ title: "Suspension Error", description: `Could not update suspension for ${u.name}.`, variant: "destructive"});
                           }
                        } else if (!db) {
                           console.info(`AuthContext: Suspending user ${u.id} (assessment listener) skipped as Firestore (db) became unavailable.`);
                        }
                    }
                  }
                  return userWithRatings;
                ...
                if(isMounted) {
                    ...
                    if (currentUserData) {
                      const updatedCurrentUser = reEnrichedUsers.find(ru => ru.id === currentUserData.id);
                      if (updatedCurrentUser && JSON.stringify(updatedCurrentUser) !== JSON.stringify(currentUserData)) {
            ...
            if (assessmentError instanceof FirestoreError && (assessmentError.code === 'unavailable' || (assessmentError.message && assessmentError.message.includes('client is offline')))) {
              ...
              if(isMounted) toast({ title: "Network Issue", description: "Could not update ratings from new assessments. Firebase is offline.", variant: "destructive", duration: 7000});
            } else {
              console.error("Error processing assessed orders snapshot:", assessmentError);
              if(isMounted) toast({ title: "Rating Update Error", description: "Failed to update ratings from new assessments.", variant: "destructive" });
            }
          }
        }, (error) => {
          if (!isMounted) return;
        ...
        if (error instanceof FirestoreError && (error.code === 'unavailable' || (error.message && error.message.includes('client is offline')))) {
            ...
            if(isMounted) toast({ title: "Network Issue", description: `AuthContext initialization error: ${error.message}. Firebase might be offline.`, variant: "destructive", duration: 10000});
        } else {
            console.error('AuthContext: Unhandled error during initialization (setupAuth):', error);
            if(isMounted) toast({ title: "Initialization Error", description: `An unexpected error occurred: ${error.message || 'Unknown error'}`, variant: "destructive" });
        }
        if (isMounted) {
          setUser(null);
          setAllUsersList([]);
